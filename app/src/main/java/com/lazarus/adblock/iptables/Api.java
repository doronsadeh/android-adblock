...
public final class Api {
	...
	public static int runScript(Context ctx, String script, StringBuilder res, long timeout, boolean asroot) {
		...
		try {
			...
			if (runner.isAlive()) {
				...
				if (runner.exec != null) runner.exec.destroy();
				runner.exec = null;
	...
	public static boolean assertBinaries(Context ctx, boolean showErrors) {
		...
		try {
			...
			if (changed) {
				Toast.makeText(ctx, "Binary files installed!", Toast.LENGTH_LONG).show();
			}
		} catch (Exception e) {
	...
	public static boolean runScriptAsRoot(Context ctx, String scripttorun, boolean showErrors)
	{
		final StringBuilder res = new StringBuilder();
		try {
			assertBinaries(ctx, showErrors);
