...
public class LocalCommunicationRunnable extends CommunicationRunnable {

	public LocalCommunicationRunnable(String name) throws AdblockException {
		super(name);
	}
	 ...
	 * 
	 * returns the local channel of a connection.
	...
	protected SelectableChannel getCommReadChannel(Connection connection) {
		return connection.getLocalChannel();
	}
	 ...
	 * connection to read from the local channel.
	 * this method is called when the local channel associated with
	 * the key is ready for read.
	 */
	@Override
	protected void read(SelectionKey key) {
		/*
		 * the local channel is ready for read
		 */
		Connection conn = (Connection)key.attachment();
			
		conn.readFromLocalChannel();
	}
	 ...
	 * 
	 * setting the super class's buffer as the local channel buffer in the
	...
	public void registerConnection(Connection connection) throws AdblockException {
		connection.setLocalChannelBuffer(byteBuffer);
